...
## 5. Monitoring Architecture (Finalized)

### 4 Health Checks (run as SageMaker Processing Job)

| Check | What It Measures | Metric |
|-------|-----------------|--------|
| **Data Drift** | KS test per feature vs training baseline, grouped into buckets | `DataDrift/{bucket}` |
| **Structural Break** | CUSUM on price series for regime change detection | `StructuralBreak/{ticker}` |
| **Model Disagreement** | Prediction divergence across 3 architectures (coeff. of variation) | `ModelDisagreement/{ticker}` |
| **Prediction Anomaly** | Predictions outside historical bounds (2 std) | `PredictionAnomaly/{ticker}` |

### 5 CloudWatch Alarms (grouped by feature bucket + global)

| Alarm | Features | Threshold |
|-------|----------|-----------|
| `futures-drift-price-trend` | OHLCV, MA, EMA, KAMA, WMA, MidPrice, TSF | KS > 0.15 |
| `futures-drift-momentum` | BOP, CMO, MFI, ROC, WILLR | KS > 0.15 |
| `futures-drift-volume` | Volume, AD, OBV | KS > 0.15 |
| `futures-drift-volatility` | NATR, ATR, TRANGE | KS > 0.15 |
| `futures-drift-global` | All 20 features aggregated | KS > 0.15 |

### Baseline

`files/models/{ticker}/baseline_stats.json` -- per-feature distribution stats from training data.
Generated by `scripts/compute_baseline_stats.py`.

---

## 6. CI/CD Pipeline Architecture (Finalized)

```
[S3 Data] → Preprocess → Train → Evaluate → Condition (MSE ≤ threshold?) → Register or Fail
```

- **Condition threshold**: MSE ≤ 0.005 (production), MSE ≤ 0.05 (validation)
- **Validation run**: epochs=5, single ticker (CL=F), single model (lstm)
- **Model Registry groups**: `futures-cl-models`, `futures-gc-models`
...
## 8. Agent Team Operating Rules (if using multi-agent)
